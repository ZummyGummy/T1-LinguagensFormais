...
class RegexToDFA(RegexVisitor):
    ...
    def convert(self, node: RegexNode) -> DFA:
        ...
        states = frozenset(map(state_name, marked))
        final_index = self.__term_count - 1
        finals = frozenset(map(state_name, filter(lambda s: final_index in s, marked)))
        return DFA(state_name(first), states, finals, transitions)
